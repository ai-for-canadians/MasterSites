...
import { BadgeCheck, Activity, AlertTriangle, BookOpen } from 'lucide-react';
                ...
                <article className="mx-auto max-w-4xl px-6 lg:px-8">
                    ...
                    <header className="mb-12 border-b border-slate-800 pb-12">
                        <div className="flex flex-wrap items-center gap-4 mb-6">
                            ...
                            <span className="text-sm font-medium text-slate-400 bg-slate-800 px-2 py-1 rounded-full uppercase tracking-wider border border-slate-700">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            ...
                            <section>
                                ...
                                <div className="prose prose-invert max-w-none">
                                    ...
                                    {intervention.keyStudies.length > 0 && (
                                        <div className="mt-8">
                                            <h3 className="text-lg font-semibold text-white mb-4">Key Studies</h3>
                                            <ul className="space-y-2">
                                                {intervention.keyStudies.map(study => (
                                                    <li key={study} className="text-slate-400 bg-slate-900/50 p-2 rounded">{study}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    )}
                        ...
                        <div className="space-y-8">
                            ...
                            {(intervention.dosage || intervention.protocol) && (
                                <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                                    <h3 className="text-lg font-bold text-white mb-4">
                                        {intervention.protocol ? 'Protocol' : 'Dosage'}
                                    </h3>

                                    {intervention.dosage && (
                                        <div className="space-y-4">
                                            <div>
                                                <div className="text-xs text-slate-500 uppercase tracking-wider mb-1">Recommended</div>
                                                <div className="text-white font-medium">{intervention.dosage.recommendation}</div>
                                            </div>
                                            {intervention.dosage.timing && (
                                                <div>
                                                    <div className="text-xs text-slate-500 uppercase tracking-wider mb-1">Timing</div>
                                                    <div className="text-slate-300">{intervention.dosage.timing}</div>
                                                </div>
                                            )}
                                            {intervention.dosage.notes && (
                                                <div className="bg-blue-500/10 p-3 rounded-lg mt-4">
                                                    <p className="text-sm text-blue-300">{intervention.dosage.notes}</p>
                                                </div>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    {intervention.protocol && (
                                        <div className="space-y-4">
                                            <div className="text-white font-medium">{intervention.protocol.title}</div>
                                            <ul className="list-disc list-inside space-y-2 text-sm text-slate-300">
                                                {intervention.protocol.steps.map(step => (
                                                    <li key={step}>{step}</li>
                                                ))}
                                            </ul>
                                        </div>
                            ...
                            <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                                ...
                                <div className="space-y-6">
                                    ...
                                    {intervention.safetyProfile.contraindications.length > 0 && (
                                        <div>
                                            ...
                                            <ul className="space-y-1">
                                                {intervention.safetyProfile.contraindications.map(ui => (
                                    ...
                                    {intervention.safetyProfile.sideEffects.length > 0 && (
                                        <div>
                                            <div className="text-xs text-slate-500 uppercase tracking-wider mb-2">Common Side Effects</div>
                                            <ul className="space-y-1">
                                                {intervention.safetyProfile.sideEffects.map(se => (
                            ...
                            {/* Biomarkers */}
                            {intervention.relatedBiomarkers.length > 0 && (
                                <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                                    <h3 className="text-lg font-bold text-white mb-4">Related Biomarkers</h3>
                                    <ul className="space-y-4">
                                        {intervention.relatedBiomarkers.map(bio => (
                                            <li key={bio.name}>
                                                <div className="text-sm font-medium text-white">{bio.name}</div>
                                                <div className="text-xs text-slate-400">{bio.description}</div>
                                                {bio.targetRange && (
                                                    <div className="text-xs text-emerald-400 mt-1">Target: {bio.targetRange}</div>
                                                )}
                                            </li>
                                        ))}
                                    </ul>
